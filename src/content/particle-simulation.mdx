...
<aside className="my-6 rounded-md border-l-4 border-amber-400 bg-amber-50 p-4">
    <div className="mx-2"><strong>Aside - Efficient Particle Storage:</strong> Within my
    spatial hash grid implementation, I needed a way to efficiently store and manage
    particles as they moved between cells. Storing a vector of particle references or
    pointers seems like the most straightforward approach, but it runs into a few
    issues since references can’t be reseated, and raw pointers would dangle the
    moment the global particle array resizes or compacts. Instead, each cell stores
    indices into a single global particle array. Doing this provides stable identifiers
    and allows for a constant-time erase via the classic swap-and-pop trick: find the
    index in the cell’s small vector, swap with the last element, and pop. 
    Here's how I went about implementing this. </div>
</aside>
...
<ol> 
  ...
  On a similar note, it is obvious to see that checking every cell during every iteration is 
  awfully slow. To address this, we can implement a sparse-row pruning technique. The idea
  ...
  **A few implementation details that matter for throughput and correctness:**
    ...
    with your half-stencil and two-stripe passes, no two tasks write the same particles in a pass,
    so the collision phase remains lock-free.

  <aside className="mt-6 rounded-md border-l-4 border-amber-400 bg-amber-50 p-4">
    <div className="mx-2"><strong>Aside - Wavefront Scheduling:</strong> I didn't see any significant
    improvement in performance with multithreading until I changed what each task looked like. This
    involved giving each worker a contiguous band of rows (e.g., 2–8 rows) and processing them left-to-right.
    Then do a second pass offset by one row so cross-row pairs that straddle band boundaries are covered
    without duplication. In practice this looks like two stripes: pass A handles rows $$$[a, a + 2, \cdots, a + 2k - 2]$$$;
    pass B handles $$$[a + 1, a + 3, \cdots, a + 2k - 1]$$$; Each band is one coarse task. This led to
    fewer tasks in the queue compared to each cell being its own task and removing the need for locks within
    each cell since there is no possibility of cross-cell interference using the two-stripe approach.</div>
  </aside>
  ...
  void collideWavefront(uint32_t rowsPerTask = 4) {
      ...
      submitStripe(1); // rows [1..k], [k+1..2k], ...
  }
```
